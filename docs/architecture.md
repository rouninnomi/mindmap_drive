...
- アクセストークンは`sessionStorage`に有効期限とともに保持する(タブを閉じれば消える。ユーザーフィードバックにより`localStorage`等への非永続化から変更)。ページ再読み込み時はまずこの保存済みトークンが有効か確認し、有効ならそれを使って再ログイン不要にする
- 保存済みトークンが無い/期限切れの場合は`prompt: ''`による無言の再認可を試みるが、GISの実装上ポップアップを使うため、ページ読み込み時のようなユーザー操作を伴わないタイミングではブラウザのポップアップブロックによりほぼ確実に失敗する(その場合は「Googleでログイン」ボタンを表示し、ユーザー操作による明示的なクリックを起点に`prompt: 'consent'`のポップアップを許可させる)
- リフレッシュトークンは扱わない(トークンクライアント方式のため)。アクセストークンの実際の有効期限(`expires_in`)を`sessionStorage`側で追跡し、期限切れなら上記と同様に再認可を試みる→失敗時は再ログイン導線
...
- 画像添付ファイルは、アプリフォルダ直下がノート整理されず無秩序に埋まらないよう、`images/<mapId>/`というマップごとのサブフォルダ配下に個別ファイルとして保存する(ユーザーフィードバックにより追加。`ensureFolder`で親フォルダごとに検索→無ければ作成)。`Attachment.driveFileId` で参照する。ファイル名はUUIDベースで衝突を避ける
...
- **テキスト編集は文字入力のたびにドメインへコミットしない**(連続して文字を打っている最中に自動保存が頻繁に挟まって煩わしいというユーザーフィードバックにより変更)。ノードのテキストは`Enter`/`Tab`/`Esc`/↑↓/Undo/Redoなどそのノードを離れる操作の直前、および`blur`時にのみコミットされる(`MapEditorPage.tsx`冒頭コメント・`MindMapCanvasNode.tsx`の`isTextCommitTriggerKey`参照)。副次効果として、Undo/Redoの粒度も1文字ずつではなく1回の入力区切りごとになった
- 加えて、`visibilitychange`(タブが非表示になる)・`beforeunload` のタイミングでダーティ状態なら即座に保存を試みる(ベストエフォート。ネットワーク遅延等で失敗する可能性は残るが、個人用途では許容する)。上記の変更により、まだコミットされていない入力中の文字を保存前に取りこぼさないよう、保存前にフォーカス中の要素を明示的に`blur()`してコミットを強制してから保存する(`useMindMapEditor.ts`)
