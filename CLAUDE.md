...
- **公開後、実際の利用の中でのユーザーフィードバックにより追加した機能・修正(2026-09-19〜20)**
  ...
  - 選択中の`Ctrl+C`/`Ctrl+X`/`Ctrl+V`でノードのコピー・切り取り・貼り付けに対応(アプリ内蔵クリップボード。`Node.cloneWithNewIds`/`MindMap.deleteNodes`をドメイン層に新規追加。貼り付けは対象ノードの子として末尾に挿入(`MindMap.pasteAsChild`。`Tab`での子ノード追加と対になる形。対象が折りたたまれていれば`Tab`と同様に展開してから貼り付ける)、切り取りは1回のUndo単位。複数選択(`Ctrl+クリック`/`Shift+クリック`)中も兄弟の並び順を保ったまままとめてコピー・切り取り・貼り付けできる。文字入力中は横取りせず`<input>`のネイティブなテキストコピー&ペーストに委ねる。ユーザーフィードバックにより追加、claude-in-chromeでの結合テストで動作確認済み。当初は対象ノードの直後に新しい兄弟として挿入する仕様(`MindMap.pasteAfter`)にしていたが、ユーザーから「兄弟ではなく対象ノードの子として貼り付けたい」とのフィードバックを受け`pasteAsChild`に変更した)
...
- **単一ノードの`Ctrl+→`展開を、押すたびに1階層ずつ深く展開できるよう変更(2026-09-23)**: 従来は選択ノードが折りたたまれている場合のみ直下の子まで展開し、展開済みの状態で`Ctrl+→`を押しても何も起きなかった。選択ノードを変えずに`Ctrl+→`を押し続けることで子→孫→…と1階層ずつ掘り下げて展開できる`MindMap.expandNextLevel`をドメイン層に新規追加(木構造上最も浅い「まだ折りたたまれて子を持つノード」の階層を一括展開する)。展開しきった状態で連打しても無駄なUndoエントリを積まないよう、読み取り専用の`MindMap.hasMoreToExpand`で呼び出し前にガードしている。ユーザーフィードバックにより追加、claude-in-chromeでの結合テストで動作確認済み
- **自動保存が効かない時の保険として、ツールバーに「JSONをコピー」「JSONを新しいタブで開く」ボタンを追加(2026-09-23)**: ユーザーから「アプリの挙動がおかしい時に自動保存が働いておらず、内容が何度か消えた」というフィードバックを受けて追加。`MindMapEditingService.exportJson()`が、保存済みかどうかに関わらず現在メモリ上にある内容(未保存の変更も含む)をDrive保存と同じJSONスキーマ(`infrastructure/drive/mindMapJson.ts`の`mindMapToJson`を再利用)で文字列化する。コピーは`navigator.clipboard.writeText`、新しいタブで開く方は`Blob`+`URL.createObjectURL`を`window.open`する実装(Chromeの組み込みJSONビューアで読みやすく表示され、そこからも手動でコピーできる)。claude-in-chromeでの結合テストで両方の動作確認済み
- **自動保存が失われる根本原因の特定とローカルドラフトによる自動復旧を追加(2026-09-23)**: 上記JSONレスキューボタン追加後、ユーザーから「私(Claude)と一緒に作業中に起きている気がする」との指摘を受けて特定。ローカル開発サーバー(Vite)で、`MindMap.ts`や`MindMapEditingService.ts`等Reactコンポーネント以外のファイルをClaudeが編集すると、Fast Refreshできずページ全体がリロードされる。このとき`beforeunload`で自動保存(Drive APIへの非同期通信)を試みてはいるが、完了を待たずにページが破棄されるため、デバウンス中(1.5秒)の未保存分が失われていた(本番のCloudflare Pagesでは起きない、開発時特有の問題と判明)。対応として、編集のたびに`localStorage`へ同期的にドラフトを書き込む仕組みを`MindMapEditingService`に追加(`persistDraft`/`clearDraft`/`readNewerDraft`。Drive保存成功時に消す)。`load`時にDrive側より新しいドラフトが残っていれば、`MapEditorPage.tsx`に「自動保存されなかった可能性のある変更が見つかりました。復元しますか?」というバナーを表示し、`restoreDraft`/`discardDraft`で選ばせる(黙って上書きしない。他端末で後から編集されている可能性があるため)。バナー表示中に気付かず編集してしまってもドラフトを上書きしない保護も追加。ユニットテスト7件追加(vitestのデフォルト実行環境に`localStorage`が無いため、テスト用に最小限のメモリ実装を用意)、claude-in-chromeでlocalStorageに擬似的な新しいドラフトを仕込んで再読み込みし、バナー表示→復元→Drive再保存→再読み込みで消える、および破棄の両経路を結合テストで動作確認済み
- **上記ドラフト復旧が実際には機能しなかった事例と、その原因である「開けっ放しのタブが古いJSのまま」問題への対応(2026-09-23)**: ドラフト復旧機能を追加した直後、本番(Cloudflare Pages)で再度自動保存漏れによる内容消失が発生。原因を切り分けたところ、コードのバグではなく、ユーザーが長時間開けっ放しにしていたタブが、今回の一連の修正(JSONレスキューボタン・ローカルドラフト復旧)がデプロイされる前のJavaScriptのまま動き続けていたことが判明(ページをリロードしない限り、SPAはデプロイ後の新しいコードを一切拾わない)。再発防止として「新しいバージョンがあります」の自動検知を追加した:
  - `vite.config.ts`にビルドごとに変わる`buildId`(タイムスタンプ)を生成するプラグインを追加。`define`で実行中のJSバンドル自身に焼き込む(`__APP_BUILD_ID__`。`src/vite-env.d.ts`で型宣言)と同時に、`generateBundle`フックで同じ値を`dist/version.json`としても書き出す(`public/_headers`に`/version.json`用の`Cache-Control: no-cache`を追加し、CDN/ブラウザキャッシュで古い内容が返り続けないようにした)
  - `useNewVersionAvailable`フック(`src/presentation/hooks/`)が`/version.json`を定期ポーリング(5分間隔)+タブのバックグラウンド復帰時(`visibilitychange`)に取得し、実行中のバンドルの`__APP_BUILD_ID__`と食い違えば新バージョンありと判定する。`App.tsx`の`AuthenticatedApp`に「新しいバージョンがあります。開けっ放しのタブは修正が反映されていない状態です。[再読み込み]」というバナーを常設。自動リロードはしない(ローカルドラフト機能があるので手動リロードしても安全なはずだが、それでもユーザーの判断を挟む)
  - claude-in-chromeで`dist/version.json`の中身を書き換えて意図的にビルドID不一致を作り、バナー表示・再読み込みボタンでの解消を確認済み(タブが非アクティブだと`visibilitychange`側の再確認は発火しない=`document.visibilityState`を正しく見ている、という点も併せて確認)
- **JSONインポート機能を追加(2026-09-23)**: 上記JSONレスキュー(コピー/新しいタブで開く)は退避するだけで、取り込んで戻す手段が無かったため、ユーザーフィードバックにより追加。マップ一覧画面(`MapListPage.tsx`)に「JSONから読み込む」ボタン(`<input type="file" accept="application/json">`)を追加。`MindMapCatalogService.importFromJson`が、`mindMapJson.ts`に新規追加した`parseMindMapJson`(構文エラー・スキーマ不一致を分かりやすいエラーメッセージにする)でパースした上で、`repository.create`でまず新しいDriveファイル(=新しいid)を確保し、そこへ`MindMap.restoreSnapshot`(Undo/Redo用の仕組みを「与えたname/root/updatedAtへまるごと置き換える」目的に転用)で内容を差し替える形で新規マップとして取り込む(インポート元JSON内の`id`は別ファイルを指していたものなのでそのまま使わない)。画像添付はJSON側に実データを含まず`driveFileId`参照のみのため、参照先の画像ファイル(`images/<元のmapId>/`配下)さえ残っていればインポート後も正常に表示される(元マップを削除しても画像フォルダ自体は削除されないため問題ない。ユーザーからの質問に答える過程で確認)。Google Drive上のJSONを直接インポートする専用フロー(Google Picker API等)は今回は作らず、Driveからローカルへダウンロードしてこの経路で読み込む形とした。claude-in-chromeで正常系(取り込んだ内容が新しいマップとして開ける)・異常系(壊れたJSON・スキーマ不一致で分かりやすいエラーが出てマップが作られない)を確認済み
- **ローカルドラフト復旧バナーの「復元する」ボタンが実際にはクリックできないバグを修正(2026-09-23)**: ユーザーから「なぜか押せない」と報告を受けて発覚。`.mindmap-canvas`が`position: absolute; inset: 0`で`.map-editor-page`いっぱいに重なる実装のため、`position`/`z-index`を指定していない`.draft-recovery-banner`(および`.new-version-banner`)は、DOM上は先に来ていてもCSSのスタッキング順では`.mindmap-canvas`の下に埋もれ、見た目は表示されていてもクリックを透過して奪われてしまっていた(既存の`.toolbar`が`position: sticky; z-index: 1`を指定して同じ問題を回避していたのが唯一の前例だったが、バナー追加時に同じ対応を入れ忘れていた)。両バナーに`position: relative; z-index: 1`を追加して解消。claude-in-chromeで実際のマウスクリック(座標クリック)でボタンが機能することを確認済み(修正前は座標クリック・要素ref経由のクリックいずれも失敗し、JS経由の`.click()`呼び出しでしか回避できていなかった=環境依存の自動化不具合ではなく実際のCSSバグだったと判明)
