...
- **公開後、実際の利用の中でのユーザーフィードバックにより追加した機能・修正(2026-09-19〜20)**
  ...
  - 選択中の`Ctrl+C`/`Ctrl+X`/`Ctrl+V`でノードのコピー・切り取り・貼り付けに対応(アプリ内蔵クリップボード。`Node.cloneWithNewIds`/`MindMap.pasteAfter`/`MindMap.deleteNodes`をドメイン層に新規追加。貼り付けは対象ノードの直後に新しい兄弟として挿入、切り取りは1回のUndo単位。複数選択(`Ctrl+クリック`/`Shift+クリック`)中も兄弟の並び順を保ったまままとめてコピー・切り取り・貼り付けできる。文字入力中は横取りせず`<input>`のネイティブなテキストコピー&ペーストに委ねる。ユーザーフィードバックにより追加、claude-in-chromeでの結合テストで動作確認済み)
